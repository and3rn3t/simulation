        ...
        if (importMatches) {
          for (const importLine of importMatches) {
            ...
            if (pathMatch) {
              ...
              if (
                importPath.includes('UltimatePatternConsolidator') ||
    ...
    const report = {
      ...
      summary: {
        success: this.results.postCheck
          ? Object.values(this.results.postCheck).every(check => check.success)
          : false,
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        log('Usage:', 'info');
        log(
          '  node deduplication-safety-auditor.cjs pre-check     # Run before deduplication',
          'info'
        );
        log(
          '  node deduplication-safety-auditor.cjs post-check    # Run after deduplication',
          'info'
        );
        log(
          '  node deduplication-safety-auditor.cjs full-audit    # Interactive full audit',
          'info'
        );
        log('', 'info');
        log(
          'This tool helps prevent code corruption during automated deduplication operations.',
          'info'
        );
        break;
    }
