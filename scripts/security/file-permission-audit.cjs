...
function log(message, type = 'info') {
  ...
  console.log(`${colors.bright}[${timestamp}]${colors.reset} ${icon} ${typeColors[type]}${message}${colors.reset}`);
}

/**
 * Find files with specific patterns
 */
function findFiles(directory, extensions = ['js', 'mjs', 'cjs', 'ts'], excludeDirs = ['node_modules', '.git', 'dist', 'coverage']) {
  const files = [];
  ...
  for (const file of jsFiles) {
    try {
      ...
      if ((hasWriteFileSync || hasCopyFileSync || hasCreateWriteStream) && !hasChmodSync) {
        ...
        const hasSecureWrapper = content.includes('secureFileCreation') || 
...
function auditDockerFiles() {
  ...
  const dockerFiles = ['Dockerfile', 'Dockerfile.dev', 'docker-compose.yml', 'docker-compose.override.yml'];
  const issues = [];
...
function generateSecurityReport(results) {
  ...
  report.summary.score = ((report.summary.passedChecks / report.summary.totalChecks) * 100).toFixed(1);
