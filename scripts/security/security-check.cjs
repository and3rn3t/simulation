...
function checkSecretsManagement() {
  ...
  log(
    "1. Go to: https://github.com/and3rn3t/simulation/settings/secrets/actions",
    "info"
  );
...
function checkPackageJsonSecurity() {
  ...
  const securityScripts = ["security:audit", "security:scan", "security:fix"];
  ...
  const securityDeps = ["@types/node", "typescript", "eslint"];
  ...
  log(
    `Security score: ${securityScore}/${securityScripts.length + 1 + securityDeps.length}`,
    "info"
  );
...
function generateSecurityReport() {
  ...
  const reportPath = path.join(projectRoot, "reports", "security-report.json");
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.chmodSync(reportPath, 0o644); // Read-write for owner, read-only for group and others
