...
function convertToESModules(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#!') || (line.startsWith('/**') || line.startsWith(' *') || line.startsWith(' */') || line.trim().startsWith('//'))) {
      newLines.push(line);
      continue;
    }
    ...
    if (!importsAdded && line.trim() !== '') {
      ...
      const needsChildProcess = content.includes("require('child_process')") || content.includes('require("child_process")');
      const needsHttps = content.includes("require('https')") || content.includes('require("https")');
    ...
    if (line.includes("const fs = require('fs')") || 
    ...
    if (line.includes('module.exports')) {
      convertedLine = line.replace(/module\.exports\s*=\s*{/, 'export {')
    ...
    if (line.includes('require.main === module')) {
      convertedLine = line.replace('require.main === module', `import.meta.url === \`file://\${process.argv[1]}\``);
      console.log(`  🔄 Converted: ${line.trim()} → ${convertedLine.trim()}`);
      updated = true;
    }
