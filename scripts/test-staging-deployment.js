  ...
  try {
    ...
    if (hasDevelop) {
      ...
      try {
        ...
        const localCommit = execSync('git rev-parse develop', { encoding: 'utf8' }).trim();
        const remoteCommit = execSync('git rev-parse origin/develop', { encoding: 'utf8' }).trim();
        
        console.log(`Local/remote sync: ${localCommit === remoteCommit ? 'Synced ✅' : 'Out of sync ⚠️'}`);
...
function testStagingWorkflow() {
  ...
  try {
    execSync('git add staging-test.txt', { stdio: 'ignore' });
    execSync(`git commit -m "test: Staging deployment test - ${timestamp.split('T')[0]}"`, { stdio: 'ignore' });
  ...
  try {
    fs.unlinkSync(testFilePath);
    execSync('git rm staging-test.txt', { stdio: 'ignore' });
    execSync('git commit -m "cleanup: Remove staging test file"', { stdio: 'ignore' });
