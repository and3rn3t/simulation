...
function generateLabels() {
  const labels = [
    // Priority labels
    { name: 'priority:critical', color: 'd73a49', description: 'Critical priority - immediate attention needed' },
    { name: 'priority:high', color: 'ff6b35', description: 'High priority - important feature or fix' },
    { name: 'priority:medium', color: 'fbca04', description: 'Medium priority - standard development work' },
    { name: 'priority:low', color: '28a745', description: 'Low priority - nice to have' },
    
    // Feature area labels
    { name: 'area:ecosystem', color: '1f77b4', description: 'Ecosystem and organism simulation features' },
    { name: 'area:education', color: 'ff7f0e', description: 'Educational content and tutorials' },
    { name: 'area:performance', color: '2ca02c', description: 'Performance optimization and efficiency' },
    { name: 'area:ui', color: 'd62728', description: 'User interface and user experience' },
    { name: 'area:infrastructure', color: '9467bd', description: 'Build, deployment, and infrastructure' },
...
- [ ] Documentation complete`
  ...
  initialIssues.forEach((issue, index) => {
    const filename = `${String(index + 1).padStart(3, '0')}-${issue.title.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-')}.md`;
