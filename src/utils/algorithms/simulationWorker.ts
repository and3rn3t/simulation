...
class PopulationPredictor {
  ...
  static predictLogisticGrowth(data: PopulationPredictionData): number[] {
    ...
    const avgGrowthRate = organismTypes.reduce((sum, type) => sum + type.growthRate, 0) / organismTypes.length;
    const intrinsicGrowthRate = avgGrowthRate * 0.01; // Convert percentage to decimal
    
    // Apply environmental modifiers
    const modifiedGrowthRate = intrinsicGrowthRate * this.getEnvironmentalModifier(environmentalFactors);
   ...
   */
  private static calculateCarryingCapacity(factors: PopulationPredictionData['environmentalFactors']): number {
   ...
   */
  private static getEnvironmentalModifier(factors: PopulationPredictionData['environmentalFactors']): number {
...
class StatisticsCalculator {
   ...
   */
  private static findClusters(organisms: StatisticsData['organisms'], radius: number): { x: number; y: number; count: number }[] {
   ...
   */
  private static calculateDispersion(organisms: StatisticsData['organisms'], canvasWidth: number, canvasHeight: number): number {
    const gridSize = 50;
    const gridWidth = Math.ceil(canvasWidth / gridSize);
    const gridHeight = Math.ceil(canvasHeight / gridSize);
    const counts: number[] = [];
...
self.onmessage = function(e: MessageEvent<WorkerMessage>) {
  ...
  try {
    ...
    const response: WorkerResponse = {
      id,
      type: type.replace('PREDICT_', 'PREDICTION_').replace('CALCULATE_', 'CALCULATION_').replace('BATCH_', 'BATCH_') as any,
      data: result
    };
