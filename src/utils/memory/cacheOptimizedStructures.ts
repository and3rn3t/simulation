...
export class OrganismSoA {
  ...
  addOrganism(
    ...
    ifPattern(this.size >= this.capacity, () => { this.resize();
     });
  ...
  removeOrganism(index: number): void {
    ifPattern(index < 0 || index >= this.size, () => { return;
     });
  ...
  updateAge(index: number, deltaTime: number): void {
    ifPattern(index >= 0 && index < this.size, () => { const currentAge = this.age?.[index];
  ...
  markReproduced(index: number): void {
    ifPattern(index >= 0 && index < this.size, () => { this.reproduced?.[index] = 1;
     });
  }
  ...
  getOrganismType(index: number): OrganismType | null {
    ifPattern(index < 0 || index >= this.size, () => { return null;
     });

    const typeIdx = this.typeIndex?.[index];
    ifPattern(typeIdx === undefined || typeIdx < 0 || typeIdx >= this.organismTypes.length, () => { return null;
     });
  ...
  canReproduce(index: number): boolean {
    ifPattern(index < 0 || index >= this.size, () => { return false;
     });

    const type = this.getOrganismType(index);
    ifPattern(!type, () => { return false;
     });
  ...
  shouldDie(index: number): boolean {
    ...
    const type = this.getOrganismType(index);
    ifPattern(!type, () => { return true; // If we can't determine type, consider it dead
     });
  ...
  getOrganism(index: number): Organism | null {
    ...
    const type = this.getOrganismType(index);
    ifPattern(!type, () => { return null;
     });

    const x = this.x?.[index];
    const y = this.y?.[index];
    ifPattern(x === undefined || y === undefined, () => { return null;
     });
  ...
  fromOrganismArray(organisms: Organism[]): void {
    ...
    // Ensure capacity
    ifPattern(organisms.length > this.capacity, () => { this.capacity = organisms.length * 2;
      this.allocateArrays();
     });
  ...
  toOrganismArray(): Organism[] {
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      ifPattern(organism, () => { organisms.push(organism);
       });
    }
