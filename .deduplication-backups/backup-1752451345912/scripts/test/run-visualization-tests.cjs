...
function main() {
  console.log(
    `${colors.bright}Enhanced Visualization & User Preferences Test Suite${colors.reset}`
  );
  console.log(
  ...
  if (!implementationComplete) {
    console.log(
      `${colors.yellow}⚠️  Implementation appears incomplete. Some tests may fail.${colors.reset}\n`
    );
  }
  ...
  if (exitCode === 0) {
    console.log(
      `${colors.green}1. ✅ All tests passed - features are ready for production${colors.reset}`
    );
    console.log(`${colors.green}2. ✅ Integration with main simulation can proceed${colors.reset}`);
    console.log(
      `${colors.green}3. ✅ Demo page is available at /public/enhanced-visualization-demo.html${colors.reset}`
    );
  } else {
    console.log(`${colors.yellow}1. 🔧 Fix failing tests before proceeding${colors.reset}`);
    console.log(
      `${colors.yellow}2. 🔧 Review error messages and update implementations${colors.reset}`
    );
    console.log(
      `${colors.yellow}3. 🔧 Re-run tests after fixes: npm run test:visualization${colors.reset}`
    );
  }
